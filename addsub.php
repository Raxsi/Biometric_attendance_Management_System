...
/*$sql2 ="select * from teacher_table where teacher_name='${teacher}'";
	$ret=$db->query($sql2);
	while($row = $ret->fetchArray(SQLITE3_ASSOC) ){
		$teachid = $row['teacher_id'];
	}*/
	$sql =<<<EOD
insert into subject_table(subid,subject_name,teacher_id,semester) values('$subid','$subname','$teacher','$sem');
EOD;
$db->exec($sql);
...
<button type="button" class="btn btn-danger" onclick="location.href='listsub.php';" >Back</button>
    <div class="container" >
        ...
        <h2 class="form-heading" style="font-size: 40px;color: white">Add Subject</h2>
       ...
       <a><label for="sem">Semester</label> </a>
            <input type="textarea" class="form-control" id="sem" placeholder="SEM" name="sem">
